...
;(function () {
    ...
    Wu.prototype.showDialog = function (ops) {
        ...
        var content_input =
            '<div class="dialog-input"><input type="text" class="dialog-prompt" autofocus /></div>';
    ...
    Wu.prototype.showAction = function (ops) {
        ...
        if (menuArr.length) {
            for (i = 0, len = menuArr.length; i < len; i++) {
                h += '<div class="wu-btn wu-actionsheet-cell" data-value="' + menuArr[i].value +
                    '" style="color: ' + menuArr[i].color + '">' + menuArr[i].title + '</div>';
            };
        };
